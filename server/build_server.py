...
if not os.path.exists(python_exe):
    ...
    print(f"Please ensure the virtual environment '{VENV_NAME}' exists in the project root '{PROJECT_ROOT}'")
    print("and contains the necessary packages (including 'pyinstaller').")
    print(f"You might need to run: python -m venv {VENV_NAME} && <activate> && pip install -r requirements.txt (in the root directory)")
    sys.exit(1)
...
try:
    ...
    print(f"\nERROR: Could not run command. Is '{python_exe}' a valid Python executable in the root venv?")
    sys.exit(1)
