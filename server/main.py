...
async def process_video_endpoint(
    ...
    try:
        ...
        audio_stream.seek(0)
        transcript: dict | None = audio2transcript.audio_to_transcript(
            audio_stream, transcript_api_key
        )
        if not transcript:
        ...
        # ToDo: Extract speaker snippets from transcript, return them to the user for identification
        # ToDo: Get the speaker names from the user, construct a mapping of speaker IDs to names

        # 4. Generate Session Script
        print("Generating session script...")
        audio_stream.seek(0)
        session_script: str = transcript2session.transcript_to_session(
            transcript, speaker_name_map={}
        )
        if not session_script:
            print("Session script generation failed or produced empty result.")
            raise HTTPException(
                status_code=500,
                detail="Failed to generate session script from transcript (or result was empty).",
            )
        print("Session script generated.")

        # 5. Generate Recap
        print("Generating recap...")
        recap: str = session2recap.session_to_recap(
            session_script, llm_api_key, recap_prompt
        )
        if not recap:
            print("Recap generation failed or produced empty result.")
            raise HTTPException(
                status_code=500,
                detail="Failed to generate recap (or result was empty).",
            )
        print("Recap generated.")

        # 6. Generate Summary
        print("Generating summary...")
        summary: str = session2recap.recap_to_summary(
            recap, llm_api_key, summary_prompt
        )
        if not summary:
            print("Summary generation failed or produced empty result.")
            raise HTTPException(
                status_code=500,
                detail="Failed to generate summary (or result was empty).",
            )
...
def launch_client():
    ...
    try:
        if not webbrowser.open(url):
            print(
                f"Could not automatically open browser. Please manually navigate to {url}"
            )
    except Exception as e:
